use upgrade::schema::lib::{
    ...
    OriginRoute, Output, OutputRoute, SelectorVersion, SemaWriteInput, SemaWriteInputRoute,
    SemaWriteOutput, SignalObjectName, TraceEvent, Version, VersionLabel,
};
...
fn generated_runtime_owner_operation_projects_to_sema_write() {
    let action = NexusWork::signal_arrived(Input::force_flip(force_flip()))
        .with_origin_route(OriginRoute(19))
        .into_nexus_action();

    assert_eq!(action.origin_route(), OriginRoute(19));
    match action.root() {
...
fn generated_runtime_owner_reply_projects_back_to_signal_output() {
    ...
    .with_origin_route(OriginRoute(31))
    .into_nexus_work()
    .into_nexus_action()
    .into_signal_output();

    assert_eq!(output.origin_route(), OriginRoute(31));
